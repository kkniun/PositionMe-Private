...
public class IndoorMapManager {
    ...
    public boolean selectVenue(@Nullable String venueId) {
        ...
        // Keep floor state only when re-selecting the same venue; otherwise start from ground-ish floor.
        String previousVenueId = selectedVenueId;
        ...
        if (venue.floors.isEmpty()) {
            currentFloor = 0;
        } else if (TextUtils.equals(previousVenueId, venue.id)) {
            currentFloor = Math.min(currentFloor, Math.max(venue.floors.size() - 1, 0));
        } else {
            currentFloor = resolveFloorPosition(venue, 0);
        }
    ...
    public int getCurrentFloorNumber() {
        ...
        if (selected == null || selected.floors.isEmpty()
                || currentFloor < 0 || currentFloor >= selected.floors.size()) {
            return 0;
        }
        return selected.floors.get(currentFloor).floorIndex;
    }
    ...
    public String getCurrentFloorLabel() {
        ...
        String label = selected.floors.get(currentFloor).floorName;
        if (!TextUtils.isEmpty(label)) {
            return label;
        }
        return String.format(Locale.US, "Floor %d", selected.floors.get(currentFloor).floorIndex);
    }
    ...
    public List<String> getFloorLabels() {
        ...
        List<String> labels = new ArrayList<>(selected.floors.size());
        for (FloorModel floor : selected.floors) {
            if (TextUtils.isEmpty(floor.floorName)) {
                labels.add(String.format(Locale.US, "Floor %d", floor.floorIndex));
            } else {
                labels.add(floor.floorName);
            }
        }
        return labels;
    }
    ...
    public void setCurrentFloor(int newFloor, boolean autoFloor) {
        ...
        int targetFloorPosition;
        if (autoFloor) {
            // Auto-floor receives an estimated floor number, so map it to the nearest available floor index.
            targetFloorPosition = resolveFloorPosition(selected, newFloor);
        } else {
            // Manual switching (buttons/spinner) uses adapter position directly.
            targetFloorPosition = Math.max(0, Math.min(newFloor, selected.floors.size() - 1));
        }
        if (targetFloorPosition == currentFloor && isIndoorMapSet) {
            return;
        }
        currentFloor = targetFloorPosition;
        renderCurrentFloor();
    }
    ...
    private int resolveFloorPosition(@NonNull VenueModel venue, int floorNumber) {
        if (venue.floors.isEmpty()) {
            return 0;
        }
        // API floor indices are not guaranteed to be 0..N-1, so choose the nearest declared floor index.
        int bestPos = 0;
        int bestDistance = Integer.MAX_VALUE;
        for (int i = 0; i < venue.floors.size(); i++) {
            int dist = Math.abs(venue.floors.get(i).floorIndex - floorNumber);
